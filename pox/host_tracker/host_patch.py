...
class host_tracker (EventMixin):
  ...
  def _handle_openflow_PacketIn (self,event):
    ...
    global tab_check_join
    key = str(dpid)+'-'+str(inport)
    if macEntry is None and key in tab_check_join.keys() and tab_check_join[key] != packet.src:
        print("ATTACCO NUOVO HOST SU PORTA OCCUPATA")
        return
