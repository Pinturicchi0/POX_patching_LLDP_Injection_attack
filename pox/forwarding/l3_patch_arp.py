...
def checkArpSpoofing(packet):
    ...
    if src_mac_ethernet != src_mac_arp:
        ...
        return True
    #2. If source MAC-IP addresses mapping in ARP header not found in Main table.
    #elif forArpTable(src_ip_arp) != None and forArpTable(src_ip_arp) != src_mac_arp:
    elif src_ip_arp in main_table.keys() and main_table[src_ip_arp] != src_mac_arp:
        print("Spoofing caso 2")
        return True
    #4. If Destination MAC is Broadcast
    #       If ARP Reply and Destination MAC is Broadcast and Source IP not like Destination IP
    elif dst_mac_ethernet == 'ff:ff:ff:ff:ff:ff':
        if (packet.payload.opcode == arp().REPLY) and (src_ip_arp != dst_ip_arp):
            print('Spoofing caso 4')
            return True
        else:
            return False
